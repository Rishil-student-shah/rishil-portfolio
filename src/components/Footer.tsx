...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-border bg-card/20 py-16 mt-auto">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12 items-center">
        {/* Logo and meta */}
        <div className="flex flex-col gap-2 text-left">
          <Link to="/" className="font-mono text-lg font-bold tracking-widest text-lime">
            RISHIL SHAH
          </Link>
          <p className="text-xs text-muted-foreground font-mono">
            ENGINEERED LOGIC. SUSTAINED GROWTH.
          </p>
        </div>

        {/* Contacts */}
        <div className="flex flex-col gap-3 font-mono text-xs text-left md:items-center">
        ...
        <div className="flex flex-col gap-4 md:items-end text-left md:text-right">
          <div className="flex gap-4">
            {contactData.social_links.github && (
              <a 
                ...
                target="_blank" 
                rel="noreferrer" 
                className="text-muted-foreground hover:text-lime transition-colors"
                data-cursor="GITHUB"
              >
          ...
          <p className="text-[10px] text-muted-foreground font-mono">
            &copy; {currentYear} RISHIL SHAH. ALL SYSTEM CONSTRAINTS RESERVED.
          </p>
        </div>
      </div>
    </footer>
